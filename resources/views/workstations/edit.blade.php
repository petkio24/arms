...
@section('title', 'Редактирование ' . $workstation->name)
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Редактирование рабочей станции</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('workstations.update', $workstation) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror"
                                   id="name" name="name"
                                   value="{{ old('name', $workstation->name) }}" required>
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('inventory_number') is-invalid @enderror"
                                   ...
                                   value="{{ old('inventory_number', $workstation->inventory_number) }}" required>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror"
                                ...
                                @foreach($statuses as $value => $label)
                                    <option value="{{ $value }}"
                                        {{ old('status', $workstation->status) == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror"
                                      id="notes" name="notes" rows="3">{{ old('notes', $workstation->notes) }}</textarea>
                            @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
